...
const rotwCatalogFixture = require('../test/fixtures/rotw-base-catalog.json');
const rotwPolicy = require('../test/fixtures/rotw-policy.json');
...
const GROUP_TITLES = {
  ...
  warlockGrimoires: 'Warlock grimoires — Reign of the Warlock',
};
...
function render() {
  ...
  const hiddenGroups = { ...policy.hiddenGroups, ...rotwPolicy.hiddenGroups };
  const notableAcceptedCollisions = {
    ...policy.notableAcceptedCollisions,
    ...rotwPolicy.notableAcceptedCollisions,
  };
  const hiddenKeys = Object.values(hiddenGroups).flat();
  const lodHiddenCount = Object.values(policy.hiddenGroups).flat().length;
  const rotwHiddenCount = Object.values(rotwPolicy.hiddenGroups).flat().length;
  const lines = [
    ...
    `${lodHiddenCount} of them target the **${policy.source.ruleset}** ruleset and are pinned to d2data commit \`${policy.source.d2dataCommit}\`.`,
    `The remaining ${rotwHiddenCount} are the **${rotwPolicy.source.ruleset}** grimoire line, pinned to d2data commit \`${rotwPolicy.source.d2dataCommit}\`; those codes are not spawnable bases in a Lord of Destruction game, so hiding them changes nothing there.`,
    '',
  ...
  Object.entries(notableAcceptedCollisions).forEach(([code, tradeoffs]) => {
    lines.push(`- \`${code}\` ${catalog[code].name}: ${tradeoffs.join('; ')}`);
  });

  Object.entries(hiddenGroups).forEach(([group, codes]) => {
    lines.push(
      '',
