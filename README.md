...
Every normal belt and boot is now hidden — including Heavy Belt despite Goldwrap and Light Plated Boots despite Goblin Toe — while exceptional and elite belts and boots stay visible for String of Ears, Razortail, Thundergod's Vigor, Arachnid Mesh, Verdungo's, War Traveler, Gore Rider and valuable rares. Gauntlets are hidden despite Frostburn; Chain Gloves (Chance Guards) and Light Gauntlets (Magefist) stay visible as explicit keeps.

Heavy Gloves are an explicit aggressive tradeoff: hiding `vgl` removes the base text, but also hides unidentified Bloodfist, magic Heavy Gloves used for Blood crafting, and potentially valuable rares. With the normal quality text, Superior Heavy Gloves render as `Superior .`; **Red Superior Items** instead removes that word, leaves only the selected dot for the hidden base, and marks desirable superior drops as a red base name such as `Mage Plate`. Hunter's Guise set the precedent; hiding `dr8` also masks Aldur's Stony Gaze and rare three-socket Druid staffmod bases.
...
See the [full generated code/name/collision table](https://github.com/voc0der/d2r-loot-filter-intense/blob/main/docs/BASE_POLICY.md) for all 323 hidden bases, every named unique/set collision accepted by the policy, and the explicit keep lists. The test suite regenerates that table from pinned LoD data and fails if the implementation, catalog, collision decisions or documentation drift apart.
