...
| **Remove Superior Prefix** | Removes the shared `Superior`/`Sup` word globally while leaving each base name visible. With **Hide Unpopular Bases**, Superior Heavy Gloves composes to only the selected dot instead of `Superior .`. |
...
This is intentionally an **endgame signal-to-noise filter**, not a claim that every hidden base is literally unusable. A common base must be worth stopping for often enough to justify all of its ordinary labels. Niche or progression collisions do not rescue it. That is why Chaos Armor is hidden despite Trang-Oul's Scales, Bearded Axe despite Spellsteel, Heavy Gloves despite Bloodfist, and Mask despite being able to take three sockets.
...
Heavy Gloves are an explicit aggressive tradeoff: hiding `vgl` removes the base text, but also hides unidentified Bloodfist, magic Heavy Gloves used for Blood crafting, and potentially valuable rares. Because `Superior` is a separately composed fragment, enable **Remove Superior Prefix** too if you want Superior Heavy Gloves to become only the dot. Hunter's Guise is another explicit volume-driven exception; hiding `dr8` also masks Aldur's Stony Gaze and rare three-socket Druid staffmod bases. The other Druid pelts, every Barbarian helm, every Sorceress orb and every Necromancer head remain visible because their staffmods can outweigh the base tier.
...
Six old normal/exceptional Assassin claws are filtered because they cannot roll Assassin staffmods, cap at two sockets and have no useful collision. Every skill-bearing claw and every three-socket Mosaic/Chaos/Pattern/Plague candidate stays visible. If you disagree with an accepted tradeoff, delete its line from `UNPOPULAR_BASE_KEYS` in [mod.js](mod.js) and reinstall.

See the [full generated code/name/collision table](https://github.com/voc0der/d2r-loot-filter-intense/blob/main/docs/BASE_POLICY.md) for all 247 hidden bases, every named unique/set collision accepted by the policy, and the explicit keep lists. The test suite regenerates that table from pinned LoD data and fails if the implementation, catalog, collision decisions or documentation drift apart.
...
`Superior` is a shared quality prefix, and socketed gray is runtime state. Neither carries the base code needed to express rules such as “hide only Superior Heavy Gloves” or “hide only a socketed Hunter's Guise.” **Remove Superior Prefix** can safely blank the global quality word while retaining every desirable base name; **Black Labels to Dots** still cannot infer either state. The aggressive list hides the entire Heavy Gloves, Mask, Chaos Armor and Hunter's Guise base strings, with the false negatives documented above.
