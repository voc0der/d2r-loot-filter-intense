...
The one **Reign of the Warlock** addition is the whole grimoire line — Old Book, Tome, Codex, Compendium and Grimoire, their Burnt Text/Dark/Possessed exceptional tier, and the elite Forgotten Volume, Occult Tome, Occult Codex, Blasphemous Compendium and Blasphemous Grimoire. It is the Warlock's shield-slot off-hand, so it is class-locked and no mercenary can ever hold one, and every tier caps at two sockets. Hiding all 15 is deliberately blunt and costs real endgame items: **Ars Al'Diabolos**, Ars Tor'Baalos, Ars Dul'Mephistos, Measured Wrath and the **Horazon's Secrets** set grimoire all go with them, and a Warlock therefore never sees a book of any rarity on the ground. The one thing it does *not* cost is a runeword shell — **Vigilance** (Dol + Gul) also fits generic shields, Necromancer heads and Paladin shields, which stay visible. These codes are not spawnable bases under Lord of Destruction, so enabling this on an LoD character changes nothing.
...
See the [full generated code/name/collision table](https://github.com/voc0der/d2r-loot-filter-intense/blob/main/docs/BASE_POLICY.md) for all 336 hidden bases, every named unique/set collision accepted by the policy, and the explicit keep lists. The test suite regenerates that table from separately pinned LoD and Reign of the Warlock data and fails if the implementation, catalogs, collision decisions or documentation drift apart.
